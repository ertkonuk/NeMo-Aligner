...
class CodeVerifier:
    ...
    def _create_function_wrapper(self, code_str, fn_name, input_args):
        ...
        # If the code defines a class Solution, call the method from an instance.
        if "class Solution" in code_str:
            call_expr = f"Solution().{fn_name}({arg_list_str})"
        else:
            call_expr = f"{fn_name}({arg_list_str})"
        return f"""
import json
{code_str}
result = {call_expr}
    ...
    def _run_single_test(self, test_data, code_str, fn_name=None):
        ...
        try:
            if fn_name:
                ...
                if isinstance(test["inputs"], str):
                    all_inputs = [json.loads(line) for line in test["inputs"].split("\n")]
                else:
                    all_inputs = test["inputs"]
                
                # Handle both string and list outputs
                if isinstance(test["outputs"], str):
                    all_outputs = json.loads(test["outputs"])
                else:
                    all_outputs = test["outputs"]
                
                wrapper_code = self._create_function_wrapper(code_str, fn_name, all_inputs)
                ...
                try:
                    ...
                    passed = actual_output == all_outputs
                    return {
                        "test_number": test_idx,
                        "passed": passed,
                        "actual_output": actual_output,
                        "expected_output": all_outputs,
                    }
