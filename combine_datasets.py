...
print(len(preference), len(ifeval))

train = preference[128:] + ifeval[128:] * 10
val = preference[:128] + ifeval[:128]

with open("data/llama3_combined_train_data_balanced.jsonl", "w") as f:
    for data in train:
        jsonline = json.dumps(data)
        f.write(jsonline + "\n")

with open("data/llama3_combined_val_data_balanced.jsonl", "w") as f:
    for data in val:
        jsonline = json.dumps(data)
        f.write(jsonline + "\n")
