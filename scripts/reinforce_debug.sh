#!/bin/bash
#SBATCH -N 1 --ntasks-per-node 8 -A llmservice_modelalignment_ppo --job-name llmservice_modelalignment_rs_debug16-rlfh:42-9_critic+reward -t 4:00:00 --exclusive --gpus-per-node=8 --partition=interactive
...
RM_NEMO_FILE="/lustre/fsw/portfolios/llmservice/users/abukharin/data/checkpoints/test_rm/megatron_gpt.nemo"
...
&& python -u examples/nlp/gpt/debug_reinforce.py \
